...
class DiagnosisAgentTrial():
    ...
    def _detect_function_call(self, ai_message: str) -> bool:
        self.add_message(ai_message, self.MessageType.AI)
        ...
        try:
            ...
            tool_response_message = f'<TOOL_RESPONSE>{{"name":"{tool_name}","output":{json.dumps(result, ensure_ascii=False)}}}</TOOL_RESPONSE>'
            self.add_message(tool_response_message, self.MessageType.USER)
            print(f"👨‍💻 You: {tool_response_message}")
            return True
            ...
            self.add_message(error_message, self.MessageType.USER)
        ...
        while True:
            try:
                if not function_call:
                    ...
                    self.add_message(user_message, self.MessageType.USER)

                response = self.medgemma.give_prompt(self.messages)
...
if __name__ == "__main__":
    diagnosis_agent_trial = DiagnosisAgentTrial()
    diagnosis_agent_trial.main()
